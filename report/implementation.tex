...
Normalization is important because the features that were extracted all uses different ranges of values, by scaling these values to the same range(e.g. [-0.5,0.5]) the classifiers are more effective in their task to find image features that separates a class from the other classes.
...
\begin{itemize}
	\item \textbf{k-Nearest Neighbour}: It classifies images based on the closest training examples.[REF WHY]
	\item \textbf{Naive Bayes}: It calculates the probability of an image belonging to a class.[REF WHY]
	\item \textbf{Nearest Mean}: It calculates the mean value of a class and classifies new images based on how close it lays to the mean of a class.[REF WHY]
	\item \textbf{Support Vector Machine}: It creates a model based on a set of training examples which belongs to two classes.
	The SVM tries to divide the training examples by a clear gap and then predicts the class of a new example based on which side of the gap it lays. [REF WHY]
\end{itemize}
...
Features were selected by using the \textit{Sequential forward feature selection}[MAYBE FORMULA].
...
Selecting the most informative feature is done using the inter-intra distance of the features.
The inter-intra distance[FORMULA] value is computed by minimizing the distance within the class while maximizing the distance with the other classes.
