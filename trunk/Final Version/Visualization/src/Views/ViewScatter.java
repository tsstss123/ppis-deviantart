...
public class ViewScatter extends FeatureView {
	...
	private void mousePressedHighlight() {
		...
		if (useClass) {
			for (DataClass c : DB.DataClasses) {
				...
				d = dist(mouseX,mouseY, c.avgX / c.itemCount, c.avgY / c.itemCount);
		...
		if (DB.highlightClass == null) {
			...
			for (DataImage im : DB.DataItems)	{
				...
				if (!im.dataClass.visible) continue;
	...
	private int[] getImagePositions(DataImage im) {
		...
		pos[1] = (int) map(im.colData[col2.index], col2.minValue, col2.maxValue, plotY2, plotY1);
	...
	private void drawData() {
		...
		for (DataImage im : DB.DataItems) {
			if (dim && (im == DB.highlightNode || im.dataClass == DB.highlightClass)) continue;
	...
	private void drawDataImages(DataImage im, Boolean dim, Boolean highlight) {
		...
		if (dim) {
			stroke(color(im.dataClass.hue, 100, 15));
			colorMode(RGB, 255);
			tint(30, 255);
		} else {
