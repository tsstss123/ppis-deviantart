function [ dataset, classvector, featurevector, artistvector, featurenames, lengthoffeature ] = createArtistDataset( datastruct, target, varargin )
        ...
        for i=1:length(lengthoffeature)
            for j=1:lengthoffeature(i)
                if (lengthoffeature(i) ~= 1)
                    featurevector(indexi)={[usefeatures{i} '_' int2str(j)]};
                else
                    featurevector(indexi)={usefeatures{i}};
                end
                indexi=indexi+1;
            end
        end
